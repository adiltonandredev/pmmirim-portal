...
// Este é o componente visual obrigatório (export default)
                ...
                {students.map((student) => (
                    <div key={student.id} className={`bg-white rounded-xl shadow-sm border p-4 flex gap-4 transition-all ${student.active ? 'border-yellow-200 shadow-yellow-100' : 'border-slate-200 opacity-75'}`}>
                        ...
                        <div className="flex flex-col justify-center gap-1 border-l pl-2 border-slate-100">
                            ...
                            {/* Botão de Deletar usando a Action correta */}
                            <DeleteButton
                                action={deleteFeaturedStudent}
                                itemId={student.id}
                                itemName={student.studentName}
                                className="h-8 w-8 p-0 bg-transparent text-slate-400 hover:text-red-600 flex items-center justify-center rounded-md"
                            />
                        </div>
                    </div>
                ))}
