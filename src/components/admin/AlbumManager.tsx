...
export function AlbumManager({ initialAlbums }: { initialAlbums: Album[] }) {
  ...
  function handleEdit(album: Album) { setCurrentAlbum(album); setView('edit'); setSelectedFiles([]) }
  function handleBack() { window.location.reload() }
  ...
  async function handleSaveAlbum(formData: FormData) {
    ...
    try {
        ...
        const result = await saveAlbum(formData)
  ...
  async function handleConfirmUpload() {
    ...
    try {
        ...
        if (result && result.success) {
           toast.success(result.message || "Fotos enviadas!", { id: toastId })
           setSelectedFiles([])
           setTimeout(() => window.location.reload(), 1500)
        } else {
           // CORREÇÃO: Usando result.message em vez de result.error
           toast.error(result?.message || "Erro ao processar.", { id: toastId })
        }
              ...
              <div key={album.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden group hover:shadow-xl transition-all duration-300">
                <div className="h-52 bg-slate-100 relative overflow-hidden">
                    {album.coverImage ? (
                      ...
                      <div className="flex items-center justify-center h-full text-slate-400 bg-slate-50"><ImageIcon size={48} /></div>
                    )}
                ...
                <div className="p-5">
                    ...
                    {/* AQUI ESTAVA O ERRO DE TIPAGEM NO MAP */}
             ...
             {currentAlbum.id ? (
                <div className="mb-6 space-y-4">
                    ...
                    <div 
                        ...
                        className={`border-3 border-dashed rounded-xl p-8 flex flex-col items-center justify-center text-center cursor-pointer transition-all ${isDragging ? 'border-blue-500 bg-blue-50 scale-[1.01]' : 'border-slate-300 hover:border-blue-400 hover:bg-slate-50'}`}
                    >
