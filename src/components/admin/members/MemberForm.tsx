...
import { Save, Loader2, UploadCloud, User, X, Instagram, Mail, Calendar } from "lucide-react"
...
export function MemberForm({ member, categories }: MemberFormProps) {
  ...
  const handleSubmit = async (formData: FormData) => {
    setLoading(true)
    if (member?.id && member.image && !preview) formData.append("removeImage", "true")
    try {
      let result
      if (member?.id) {
        formData.append("id", member.id)
        if (!formData.get("removeImage") && (!formData.get("image") || (formData.get("image") as File).size === 0)) {
      ...
      if (!result.success) {
        showError("Erro", result.message || "Erro ao salvar o membro.")
      } else {
        showSuccess("Salvo!", result.message || (member?.id ? "Membro atualizado!" : "Membro adicionado!"))
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  {categories.length > 0 ? (
                    ...
                    <div className="p-2 border border-dashed border-slate-300 rounded-md text-xs text-slate-400 text-center">
                      Nenhuma categoria cadastrada.{" "}
                      <a href="/admin/institution/team/categories" className="text-blue-600 underline">Criar categorias</a>
                    </div>
                  )}
                </div>
                <div className="space-y-2">
                  <Label className="flex items-center gap-1.5"><Calendar size={13} /> Data de Aniversário</Label>
                  <Input
                    name="birthDate"
                    type="date"
          ...
          <div className="space-y-4">
            <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-3">
              ...
              <div className="relative w-full aspect-[3/4] bg-slate-50 rounded-lg overflow-hidden border-2 border-dashed border-slate-200 hover:border-blue-400 group cursor-pointer transition-colors">
                ...
                <input type="file" name="image" accept="image/png, image/jpeg, image/webp" onChange={handleImageChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20" />
              </div>
