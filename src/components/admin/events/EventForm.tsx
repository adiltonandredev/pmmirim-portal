...
export function EventForm({ event }: EventFormProps) {
    ...
    const handleSubmit = async (formData: FormData) => {
        ...
        // 1. Lógica para remover a imagem: 
        // Se o evento já existia, tinha banner, mas o preview agora é null (usuário clicou no X)
        if (event?.id && event.bannerUrl && !preview) {
        ...
        try {
            ...
            if (event?.id) {
                ...
                if (!formData.get("removeImage") && (!formData.get("bannerUrl") || (formData.get("bannerUrl") as File).size === 0)) {
