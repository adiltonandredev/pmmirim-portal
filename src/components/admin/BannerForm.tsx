...
import { updateBanner, createBanner } from "@/actions/banners"
...
export function BannerForm({ banner }: BannerFormProps) {
    ...
    const handleSubmit = async (formData: FormData) => {
        setLoading(true)
        try {
            let result: { success: boolean; message: string }; // Tipagem explícita
            if (banner?.id) {
                ...
                result = await updateBanner(formData) as any;
            } else {
                result = await createBanner(formData) as any;
            }
